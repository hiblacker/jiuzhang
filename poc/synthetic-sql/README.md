...
This slice uses the admin connection only for migrations and fixture setup; building runs as a NOLOGIN `p0_worker` role, publication as a NOLOGIN `p0_publisher` role, each reaching the database only through `SECURITY DEFINER` functions and the approval/exemption tables. Report identities are real LOGIN roles whose passwords are generated per run, sent on stdin only, and verified SCRAM-stored; no login credentials, API, authentication service or connection pool is delivered. Report identities can only read authorized views plus per-domain flow completeness (`reporting.release_flow`); raw tables, publish functions and role escalation are denied. They are trusted SQL consumers, not an arbitrary-SQL HTTP endpoint. The image's local socket auth is trust — password and TLS enforcement over TCP, query budgets, worker-vs-publisher audit hardening are still required before serving users.
...
Only PUBLISHED releases are visible. Authorization uses the database `session_user` plus a private scope table, not a caller-set tenant variable. Changing SET ROLE does not select another tenant. Zero duration denominator and incomplete history remain NULL with reason columns. Monthly distincts are recomputed within each team; cross-team distinct totals must be recomputed from authorized object details, not summed. A published domain whose flow integrity failed at build time stays queryable but is flagged incomplete in `reporting.release_flow`; serving it required an approved per-domain exemption at publication.
