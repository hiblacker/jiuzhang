...
迁移服务必须成功退出，随后 `provision-roles` 为 `bydw_control_api_login` 和 `bydw_ingestion_worker_login` 设置登录密码，API 才会以受限控制角色启动。已经执行的迁移按文件名和 SHA-256 记录；不要修改已有迁移文件，变更使用新的 `VNNN__name.sql`。失败或取消批次通过 Worker Token 调用 retry/cancel 接口，不能直接修改数据库状态。Worker 数据库角色已预留，真实 Worker 进程仍未接入。
