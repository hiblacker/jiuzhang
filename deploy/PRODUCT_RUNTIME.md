...
Node 22.23.1、Python 3.12.14、PostgreSQL 16.15、MySQL CLI 8.0.43、57 个 Linux wheel、Debian 包与基础镜像版本继续固定。前端先 `npm ci` 和构建，再打 Java 包及独立控制台；Worker 镜像包含完整采集模块。API 与 Worker 默认标签为 `0.2.0-dev.5`，控制台为 `0.2.0-dev.4`；已有标签拒绝覆盖。
