...
当前 [Compose](compose.product.yaml)提供 PostgreSQL、显式迁移、角色配置、Java 同源工作台/API、Node 接入 Worker 和 Python/dbt 模型 Worker。新版安装、初始化邀请、三条操作主流程及升级步骤以 [产品工作台手册](../docs/41-product-workbench-runbook.md)为准。
...
配置工具只创建新目录，默认不登记业务来源。API 60283 与工作台 60284 均映射 Java 8080，页面使用账号会话。初始化使用 `tools/product-bootstrap.py`，不在页面粘贴 Admin Token。

Node 22.23.1、Python 3.12.14、PostgreSQL 16.15、MySQL CLI 8.0.43、57 个 Linux wheel、Debian 包与基础镜像版本继续固定。前端先 `npm ci` 和构建，再打 Java 包；Worker 镜像包含完整采集模块。默认新镜像标签为 `0.2.0-dev.2`，已有标签拒绝覆盖。
...
已有合成容器环境可运行 `tests/product-compose-integration.py` 验证 v1 兼容；新版动态接入与浏览器流程使用 `tools/verify-product.py`。仅针对隔离合成环境运行，禁止套用在已投入使用的目录。
