...
const projects = ref<Project[]>([]),
  selectedProject = ref<Project | null>(null),
  projectId = computed(() => selectedProject.value?.id || null),
  role = computed(() => selectedProject.value?.role);
const projectSearch = ref(''),
  projectPage = ref(1),
  projectTotal = ref(0),
  projectModal = ref(false);
const menu = computed(() =>
  [
    ['home', '工作台'],
    ['systems', '接入管理'],
    ['assets', '资产目录'],
    ...(role.value === 'OWNER' || role.value === 'ENGINEER' ? [['models', '数据开发']] : []),
    ['datasets', '数据服务'],
    ['runs', '运行中心'],
    ['settings', '项目与设置'],
...
async function loadProjects() {
  ...
  const result = await api<Page<Project>>(
    `/warehouse/catalog/projects?q=${encodeURIComponent(projectSearch.value)}&limit=25&offset=${(projectPage.value - 1) * 25}`,
  );
  if (current === projectGeneration) {
    projects.value = result.items;
    projectTotal.value = result.total;
    if (!selectedProject.value) selectedProject.value = result.items[0] || null;
  }
}
async function identify() {
  ...
  if (selectedProject.value) {
    const current = identity.value.projects.find((p) => p.id === selectedProject.value?.id);
...
function activationProblem(invitation: string, password: string) {
  ...
  if (code.length !== INVITATION_LENGTH)
    return `邀请码应为 ${INVITATION_LENGTH} 位，当前 ${code.length} 位。请只复制邀请文件里 invitation 的值，不要带引号、花括号或换行。`;
  if (!password) return '请输入要设置的密码。';
  if (password.length < MIN_PASSWORD_CHARS)
    return `密码至少 ${MIN_PASSWORD_CHARS} 个字符，当前 ${password.length} 个。`;
...
async function signIn() {
  if (activate.value) {
    const problem = activationProblem(invitation.value, password.value);
    if (problem) {
      error.value = problem;
      return;
    }
...
watch(projectPage, () => void searchProjects());
watch(menu, (items) => {
  if (!items.some((m) => m.key === page.value)) page.value = 'home';
});
    ...
    <main v-if="!identity" class="login-shell">
      <n-card class="login-card" title="九章数据平台">
        ...
        <form @submit.prevent="signIn">
          ...
          /></n-form-item>
          <n-button attr-type="submit" type="primary" block :loading="busy" :disabled="!ready">{{
            activate ? '设置密码' : '登录'
          }}</n-button>
          <n-button
            text
    ...
    <n-layout v-else has-sider class="workspace">
      ...
      <n-layout content-style="padding: 28px 36px">
        <header>
          <div>
            <h1>{{ heading }}</h1>
            <span class="muted">{{ identity.identity }} · {{ identity.platformAdmin ? '平台管理员' : role }}</span>
          </div>
          <n-space align="center"
            ><n-button
              @click="
                ...
                searchProjects();
              "
              >{{ selectedProject?.name || '选择项目' }}</n-button
            ><n-button @click="signOut">退出</n-button></n-space
          >
        </header>
        <n-alert v-if="error" type="error" class="gap">{{ error }}</n-alert>
        <ProjectSettings
          v-if="page === 'settings' || (!projectId && identity.platformAdmin)"
          :project="projectId"
          :can-manage="role === 'OWNER'"
          :admin="identity.platformAdmin"
          :identity="identity.identity"
          @projects-changed="identify().catch((e) => (error = e.message))"
        />
        ...
        <template v-else>
          ...
          <SystemManager
            v-else-if="page === 'systems'"
            :project="projectId"
            :can-manage="role === 'OWNER'"
            :can-ingest="role === 'OWNER' || role === 'ENGINEER'"
          />
          <AssetCatalog v-else-if="page === 'assets'" :project="projectId" />
          <ModelWorkspace
            v-else-if="page === 'models' && (role === 'OWNER' || role === 'ENGINEER')"
            :project="projectId"
            :can-manage="role === 'OWNER'"
          />
          ...
          <RunCenter
            ...
            :project="projectId"
            :can-manage="role === 'OWNER'"
            :can-operate="role === 'OWNER' || role === 'ENGINEER'"
            :identity="identity.identity"
          />
        </template>
      </n-layout>
    </n-layout>
    <n-modal v-model:show="projectModal" preset="card" title="选择项目" style="width: min(820px, 96vw)"
      ><n-space class="gap"
        ><n-input
        ...
        /><n-button
          ...
          >搜索</n-button
        ></n-space
      ><n-data-table :columns="projectColumns" :data="projects" /><n-pagination
