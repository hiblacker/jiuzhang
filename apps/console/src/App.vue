...
const currentRoute = computed(
  () => visibleRoutes.value.find(r => r.key === route.value) ?? visibleRoutes.value[0]!,
)
const projectScoped = computed(() =>
  ['assets', 'models', 'access'].includes(currentRoute.value.key),
)
...
async function connect() {
  ...
  try {
    ...
    session.base = url
    session.token = token.value.trim()
    ...
    connectionError.value = errorText(e)
    disconnect()
  }
  finally {
...
async function refresh() {
  ...
  try {
    ...
    if (generation === session.generation || !session.connected) {
      connectionError.value = errorText(e)
    }
  }
  finally {
...
const theme = {
  common: {
    primaryColor: '#167d65',
    primaryColorHover: '#116d58',
    primaryColorPressed: '#0b5947',
    borderRadius: '4px',
    ...
    <NMessageProvider>
      <NDialogProvider>
          ...
          <aside class="sidebar" :class="{ expanded: mobileMenu }">
            ...
            <nav aria-label="主导航">
              <button
                ...
                :key="item.key"
                :class="{ active: currentRoute.key === item.key }"
                :aria-current="currentRoute.key === item.key ? 'page' : undefined"
            ...
            <div class="sidebar-bottom">
              <span class="connection-dot" :class="{ online: session.connected }" />{{
                session.connected ? '控制 API 已连接' : '未连接'
              }}<small>CONSOLE 0.2.0</small>
            </div>
          </aside>
          <div class="workspace">
            <header class="topbar">
                ...
                <NTooltip>
                  <template #trigger>
                    <NButton
                      ...
                      @click="reconnect"
                    >
                      <component :is="session.connected ? LogOut : PlugZap" :size="18" />
                    </NButton>
                  </template>重新连接
                </NTooltip>
              </div>
            ...
            <main>
              ...
              <NAlert v-if="connectionError && !login" type="error" class="section-gap">
                {{ connectionError }}
              </NAlert>
              <div v-if="session.connected && projectScoped" class="project-bar">
                <label for="project-picker">当前项目</label><NSelect
                  :value="projectId"
                  :options="projects.map((p) => ({ label: p.name, value: p.id }))"
                  :input-props="{ id: 'project-picker', 'aria-label': '当前项目' }"
                  filterable
                  @update:value="projectId = $event"
              ...
              <template v-if="session.connected">
                <OverviewView
                  ...
                  :key="`overview-${refreshKey}-${session.generation}`"
                />
                <IngestionView
                  v-else-if="['sources', 'plans', 'runs'].includes(currentRoute.key)"
                  :key="`${currentRoute.key}-${refreshKey}-${session.generation}`"
                  :view="currentRoute.key"
                />
                <AssetsView
                  v-else-if="currentRoute.key === 'assets'"
                  :key="`assets-${projectId}-${refreshKey}-${session.generation}`"
                />
                <ModelsView
                  v-else-if="currentRoute.key === 'models'"
                  :key="`models-${projectId}-${refreshKey}-${session.generation}`"
                />
                <AccessView
                  v-else
                  :key="`access-${projectId}-${refreshKey}-${session.generation}`"
                />
              </template>
              <div v-else class="disconnected">
                <Database :size="42" />
                <h2>九章数据平台</h2>
                <NButton type="primary" @click="login = true">
                  连接工作台
                </NButton>
              </div>
            </main>
        ...
        <NModal
          ...
          :close-on-esc="false"
        >
          <NAlert v-if="connectionError" type="error" class="section-gap">
            {{ connectionError }}
          </NAlert>
          <NForm label-placement="top" @submit.prevent="connect">
            <NFormItem label="控制 API 地址" :label-props="{ for: 'base-url' }">
              <NInput
                v-model:value="base"
                :disabled="connecting"
                :input-props="{ id: 'base-url', autocomplete: 'url' }"
              />
            </NFormItem>
            <NFormItem label="访问令牌" :label-props="{ for: 'token' }">
              <NInput
                ...
                :disabled="connecting"
                :input-props="{ id: 'token', autocomplete: 'off' }"
              />
            </NFormItem>
