...
export interface AuthSnapshot { base: string; token: string; generation: number }
...
export function createTransport(readSession: () => AuthSnapshot, unauthorized: () => void, fetcher: typeof fetch = fetch) {
  ...
  async function request<T>(route: string, body?: unknown, signal?: AbortSignal, csv = false): Promise<T> {
    const { generation, base, token } = readSession()
    const controller = new AbortController()
    const combined = AbortSignal.any([controller.signal, AbortSignal.timeout(15000), ...(signal ? [signal] : [])])
    pending.add(controller)
    // Response parsing is asynchronous too; recheck before changing any session or visible state.
    function current() {
      combined.throwIfAborted()
      if (generation !== readSession().generation) throw new DOMException('Session changed', 'AbortError')
    }
    try {
      current()
      ...
      if (!response.ok) {
        ...
        throw new ApiError(response.status, typeof value.code === 'string' ? value.code.slice(0, 200) : `HTTP_${response.status}`)
      }
