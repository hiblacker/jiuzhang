...
const bindings = ref<Record<string, PickedAsset>>({}),
  picker = ref(false),
  alias = ref(''),
  creating = ref(false),
  modelCode = ref(''),
  modelName = ref(''),
  chosenPackage = ref<Package | null>(null),
  importing = ref(false);
const repositories = ref<{ code: string; name: string; project_paths: string[] }[]>([]),
  repository = ref<string | null>(null),
...
const contract = computed(() =>
  creating.value ? chosenPackage.value?.contract : versions.value.find((v) => v.version === version.value)?.contract,
);
...
const columns = [
  ...
  { title: '编码', key: 'code' },
  { title: '模型版本', key: 'active_model_version' },
  { title: '当前发布', key: 'active_release_id' },
];
const packageColumns = [
  { title: 'Git 项目', key: 'project_path' },
  { title: '状态', key: 'state' },
  {
    title: '提交',
    key: 'git_revision',
    render: (r: Package) => r.git_revision?.slice(0, 12) || r.error_code || '等待 Worker',
  },
  {
    title: '操作',
    key: 'actions',
    render: (r: Package) =>
      h(
        NButton,
        {
          ...
          onClick: () => {
            chosenPackage.value = r;
            creating.value = true;
            bindings.value = {};
            modelCode.value = selected.value?.code || '';
            modelName.value = selected.value?.name || '';
            importing.value = false;
          },
        },
        () => (selected.value ? '用此包新增模型版本' : '绑定资产建立模型'),
      ),
  },
];
const buildColumns = [
    ...
    render: (r: Record<string, any>) =>
      h(NSpace, {}, () => [
        ...
        ...(['QUEUED', 'RUNNING'].includes(r.state)
          ? [h(NButton, { size: 'small', onClick: () => cancel(r.id) }, () => '取消')]
          : []),
      ]),
  },
];
const buildDetail = ref<Record<string, any> | null>(null),
...
async function refresh() {
  ...
  [versions.value, builds.value, releases.value] = await Promise.all([
    api<typeof versions.value>(route() + '/versions'),
    api<Record<string, any>[]>(route() + '/builds'),
    api<Record<string, any>[]>(route() + '/releases'),
  ]);
  selected.value = {
    ...selected.value,
    active_model_version: versions.value[0]?.version || 0,
    active_release_id: releases.value[0]?.id || null,
  };
}
...
function choose(value: PickedAsset) {
  bindings.value[alias.value] = value;
  picker.value = false;
}
async function saveModel() {
  ...
  try {
    const result = await api<{ id: number; version: number }>(`/warehouse/projects/${props.project}/models`, {
      ...
      packageId: chosenPackage.value?.package_id,
      bindings: Object.fromEntries(
        Object.entries(bindings.value).map(([a, v]) => [a, { sourceCode: v.source_code, objectName: v.name }]),
      ),
    });
    creating.value = false;
    const keep = { ...bindings.value };
    await load();
    await inspect({
      id: result.id,
      code: modelCode.value,
      name: modelName.value,
      active_model_version: result.version,
      active_release_id: selected.value?.active_release_id || null,
    });
    bindings.value = keep;
  } catch (e) {
    error.value = (e as Error).message;
...
async function build() {
  ...
  error.value = '';
  try {
    await api(route() + '/builds', {
      requestKey: crypto.randomUUID(),
      modelVersion: version.value,
      inputs: Object.fromEntries(Object.entries(bindings.value).map(([a, v]) => [a, v.id])),
    });
    await refresh();
  } catch (e) {
    error.value = (e as Error).message;
...
async function cancel(id: number) {
  try {
    await api(route() + `/builds/${id}/cancel`, {});
    await refresh();
  } catch (e) {
    error.value = (e as Error).message;
  }
...
async function publish() {
  ...
  error.value = '';
  try {
    await api(route() + `/builds/${publishBuild.value}/publish`, { reason: publishReason.value });
...
watch(repository, () => {
  projectPath.value = repositories.value.find((r) => r.code === repository.value)?.project_paths[0] || null;
});
...
<template>
  ...
  <template v-else
    ><n-space class="gap"
      ><n-button
        ...
        "
        >返回数据集目录</n-button
      ><n-button @click="refresh().catch((e) => (error = e.message))">刷新状态</n-button
      ><n-button @click="openImport">登记模型新版本</n-button></n-space
    ><n-card :title="selected.name"
      ><n-space align="center"
        ><n-select
          v-model:value="version"
          :options="versions.map((v) => ({ label: `模型版本 ${v.version}`, value: v.version }))"
          style="width: 180px"
        /><n-tag>当前发布 {{ selected.active_release_id || '尚未发布' }}</n-tag></n-space
      >
      <p>粒度：{{ contract?.grain }} · 业务域：{{ contract?.domain }}</p>
      <h3>选择本次构建的输入版本</h3>
      <n-space v-for="input in contract?.inputs" :key="input.alias" class="gap" align="center"
        ><strong>{{ input.alias }}</strong
        ><span>{{ input.sourceCode }} / {{ input.objectName }}</span
        ><n-button
          @click="
            alias = input.alias;
            picker = true;
          "
          >选择资产版本</n-button
        ><span v-if="bindings[input.alias]"
          >{{ bindings[input.alias].data_window }} · 批次 {{ bindings[input.alias].run_id }}</span
        ></n-space
      ><n-button
        type="primary"
        class="gap"
        :loading="busy"
        :disabled="Object.keys(bindings).length !== contract?.inputs.length"
        @click="build"
        >使用固定输入构建</n-button
      ><n-data-table :columns="buildColumns" :data="builds" class="gap" />
    ...
    <n-space
      ><n-form-item label="批准仓库"
        ><n-select
          ...
          :options="repositories.map((r) => ({ label: r.name, value: r.code }))"
          style="width: 220px" /></n-form-item
      ><n-form-item label="模型目录"
        ><n-select
          v-model:value="projectPath"
          :options="
            (repositories.find((r) => r.code === repository)?.project_paths || []).map((value) => ({
              label: value,
              value,
            }))
          "
          style="width: 220px" /></n-form-item
      ><n-form-item label="Git 分支、标签或提交"><n-input v-model:value="gitRef" /></n-form-item></n-space
    ><n-space class="gap"
  ...
  <n-modal v-model:show="creating" preset="card" title="绑定模型与项目资产" style="width: min(850px, 96vw)"
    ...
    ></n-space>
    <p>模型版本来自提交 {{ chosenPackage?.git_revision }}，字段、粒度与质量规则由该版本定义。</p>
    <n-space v-for="input in chosenPackage?.contract.inputs" :key="input.alias" class="gap" align="center"
      ><strong>{{ input.alias }}</strong
      ><span>需要字段 {{ input.columns.map((c) => c.name).join('、') }}</span
      ><n-button
        @click="
          alias = input.alias;
          picker = true;
        "
        >选择来源对象</n-button
      ><span>{{ bindings[input.alias]?.source_code }} / {{ bindings[input.alias]?.name }}</span></n-space
    ><n-button
      type="primary"
      :loading="busy"
      :disabled="Object.keys(bindings).length !== chosenPackage?.contract.inputs.length"
      @click="saveModel"
      >保存模型版本</n-button
    ></n-modal
  >
  <n-modal v-model:show="picker" preset="card" title="选择完整资产版本" style="width: min(1080px, 96vw)"
    ><AssetPicker
      ...
      @select="choose"
  /></n-modal>
  <n-modal v-model:show="detailVisible" preset="card" title="构建质量与依赖" style="width: min(980px, 96vw)"
    ><n-tabs
      ><n-tab-pane name="quality" tab="质量检查"
      ...
      ><n-tab-pane name="lineage" tab="节点依赖"
        ><n-data-table
          :columns="[
            { title: '节点', key: 'id' },
            { title: '依赖', key: 'dependsOn', render: (r: any) => r.dependsOn.join(' → ') },
          ]"
          :data="buildDetail?.result?.worker?.lineage || []"
        />
