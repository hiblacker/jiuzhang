...
const columns: DataTableColumns<System> = [
  {
    title: '业务系统',
    key: 'name',
    render: (row) => h(NButton, { text: true, type: 'primary', onClick: () => detail(row) }, () => row.name),
  },
  { title: '编码', key: 'code' },
  { title: '组织', key: 'organization' },
  { title: '技术责任人', key: 'technical_owner' },
  {
    title: '状态',
    key: 'lifecycle',
    render: (row) => h(NTag, {}, () => lifecycleNames[row.lifecycle] || row.lifecycle),
  },
  { title: '可见实例', key: 'instance_count' },
];
...
function open(edit = false) {
  editing.value = edit;
  const s = edit ? selected.value : null;
  Object.assign(form, {
    code: s?.code || '',
    name: s?.name || '',
    domain: s?.domain || '',
    organization: s?.organization || '',
    businessOwner: s?.business_owner || '',
    technicalOwner: s?.technical_owner || '',
    description: s?.description || '',
    maxParallel: s?.max_parallel || 2,
    expectedVersion: s?.revision || 0,
  });
  modal.value = true;
}
async function save() {
...
<template>
  ...
  <template v-else>
    <n-space class="gap"
      ><n-button @click="selected = null">返回目录</n-button
      ><n-button v-if="canManage" @click="open(true)">编辑系统信息</n-button></n-space
    >
    <OperationBar
      v-if="canManage"
      :project="project"
    ...
    <n-card :title="selected.name">
      <n-descriptions :column="3" bordered
        ><n-descriptions-item label="系统编码">{{ selected.code }}</n-descriptions-item
        ><n-descriptions-item label="业务责任人">{{ selected.business_owner }}</n-descriptions-item
        ><n-descriptions-item label="技术责任人">{{ selected.technical_owner }}</n-descriptions-item
        ><n-descriptions-item label="业务域">{{ selected.domain || '未填写' }}</n-descriptions-item
        ><n-descriptions-item label="组织">{{ selected.organization || '未填写' }}</n-descriptions-item
        ><n-descriptions-item label="登记状态">{{
          lifecycleNames[selected.lifecycle]
    ...
    <n-card title="环境实例" class="gap"
      ><template #header-extra
        ><n-button
          v-if="canManage"
          @click="
            Object.assign(instanceForm, { code: '', name: '', environment: 'TEST', purpose: '' });
            instanceModal = true;
          "
          >新增实例</n-button
        ></template
      ><n-data-table
        v-model:checked-row-keys="checkedInstances"
        :row-key="(r) => r.id"
        ...
        :targets="
          instances
            ...
            .map((i) => ({ type: 'instance', id: i.id, expectedVersion: i.revision }))
        "
        @changed="
          detail(selected);
          load();
        " /><slot name="connections" :system="selected" :instances="instances"
    /></n-card>
    <IngestionManager
      v-if="canIngest"
      :project="project"
  ...
  >
    ...
    <n-space
      ><n-form-item label="业务域"><n-input v-model:value="form.domain" /></n-form-item
      ><n-form-item label="归属组织"><n-input v-model:value="form.organization" /></n-form-item
    ></n-space>
    <n-space
      ><n-form-item label="业务责任人"><n-input v-model:value="form.businessOwner" /></n-form-item
      ><n-form-item label="技术责任人"><n-input v-model:value="form.technicalOwner" /></n-form-item
    ></n-space>
    <n-form-item label="系统并行任务上限"
      ><n-input-number v-model:value="form.maxParallel" :min="1" :max="100" /></n-form-item
    ><n-form-item label="说明"><n-input v-model:value="form.description" type="textarea" /></n-form-item
  ...
  <n-modal v-model:show="instanceModal" preset="card" title="新增环境实例" style="width: min(540px, 94vw)">
    ...
    ><n-form-item label="源系统环境"
      ><n-select v-model:value="instanceForm.environment" :options="environmentOptions" /></n-form-item
    ><n-form-item label="用途"><n-input v-model:value="instanceForm.purpose" /></n-form-item
    ><n-button type="primary" :loading="busy" @click="saveInstance">保存</n-button>
  </n-modal>
</template>
