...
const sourcePage = ref(0),
  pageSources = ref<Source[]>([]),
  filter = ref(''),
  planFilter = ref<number | null>(null);
const modal = ref(false),
  modalTitle = ref(''),
  fields = ref<Field[]>([]),
  initial = ref<Row>({}),
  context = ref<Row | null>(null);
...
async function load() {
  if (props.view === 'sources') await loadSources();
    ...
    if (props.view === 'plans') await allSources();
    else await loadRuns();
  }
}
function open(
...
function planForm(plan?: Plan) {
  ...
  open(plan ? `修改计划 · ${plan.source_code}` : '新建计划', fields, defaults, (value) => {
    ...
    return call('lake/plans', {
      ...value,
      expectedVersion:
        plan?.active_version ?? plans.value.find((p) => p.source_code === value.sourceCode)?.active_version ?? 0,
      triggerTime: String(value.triggerTime).length === 5 ? `${value.triggerTime}:00` : value.triggerTime,
    });
  });
}
    ...
    [
      {
        ...
        type: 'select',
        required: true,
        options: plans.value.map((p) => ({ label: `${p.source_code} · v${p.active_version}`, value: p.id })),
      },
...
function executionActions(row: Row): Action[] {
  ...
  if (['FAILED', 'INCOMPLETE', 'CANCELLED'].includes(String(row.state)))
    actions.push({ label: '重试', run: () => mutate('重试执行', `lake/executions/${row.id}/retry`, {}) });
  ...
  const result = object(row.result),
    assets = Array.isArray(result.assets) ? (result.assets as Row[]) : [];
  if (
    ...
    actions.push({ label: '重解析原件', run: () => mutate('重解析原件', `lake/executions/${row.id}/reprocess`, {}) });
...
<template>
  ...
  <section v-if="view === 'sources'" class="section">
    <div class="section-head">
      <h2>已登记来源</h2>
      <n-button type="primary" :disabled="loading" @click="newSource"
        ><template #icon><Plus :size="16" /></template>登记来源</n-button
      >
    </div>
    <DataGrid
      :rows="pageSources"
    ...
    />
    <div class="pager">
      <span class="muted">第 {{ sourcePage + 1 }} 页</span
      ><n-button :disabled="loading || sourcePage === 0" @click="run(() => loadSources(sourcePage - 1))"
        ><template #icon><ChevronLeft :size="16" /></template>上一页</n-button
      ><n-button :disabled="loading || pageSources.length < 50" @click="run(() => loadSources(sourcePage + 1))"
  ...
  <section v-else-if="view === 'plans'" class="section">
    <div class="section-head">
      <h2>持续入湖计划</h2>
      <n-button type="primary" :disabled="loading || !sources.length" @click="planForm()"
        ><template #icon><Plus :size="16" /></template>新建计划</n-button
      >
    </div>
    <div class="toolbar">
      <n-input v-model:value="filter" placeholder="筛选来源" :input-props="{ 'aria-label': '筛选来源' }" clearable />
    </div>
    <DataGrid
      :rows="visiblePlans"
      :loading="loading"
      ...
      :actions="
        (row) => [
          { label: '修改', run: () => planForm(row as Plan) },
          {
            ...
            run: () =>
              mutate(row.state === 'ACTIVE' ? '暂停计划' : '恢复计划', `lake/plans/${row.id}/state`, {
  ...
  <section v-else class="section">
    <div class="section-head">
      <h2>运行与恢复</h2>
      <div class="toolbar">
        <n-button :disabled="loading" @click="mutate('检查遗漏窗口', 'lake/calendar/reconcile', {})"
          ><template #icon><ListChecks :size="16" /></template>检查遗漏窗口</n-button
        ><n-button type="primary" :disabled="loading || !plans.length" @click="trigger()"
    ...
    <n-tabs v-model:value="activeTab" type="line"
      ...
      /></n-tab-pane>
      <n-tab-pane name="windows" tab="交付窗口" :tab-props="tabProps('windows')" v-bind="panelProps('windows')"
        ><DataGrid
          :rows="visibleWindows"
          :loading="loading"
          :columns="[
