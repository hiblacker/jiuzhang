...
async function load() {
  ...
  try {
    ...
    if (current === generation) {
      ...
      datasetCount.value = data.total;
      incidentCount.value = issues.total;
      incidents.value = issues.items;
      datasets.value = descriptions;
      error.value = '';
    }
  } catch (e) {
  ...
  <n-card title="数据集状态（前 5 项）" class="gap"
    ><n-data-table
      :columns="[
        ...
        { title: '新鲜度', key: 'freshness', render: (r: any) => names[r.freshness.state] },
        { title: '本轮任务', key: 'task', render: (r: any) => r.freshness.currentTask.state },
      ]"
      :data="datasets"
  /></n-card>
  <n-card title="最近待处理异常（前 5 项）" class="gap"
    ><n-data-table
      :columns="[
        { title: '系统', key: 'system_name' },
        { title: '来源', key: 'source_code' },
        { title: '数据集', key: 'dataset_name' },
        { title: '原因', key: 'category' },
      ]"
      :data="incidents"
    /><n-space class="gap"><n-tag>当前项目授权范围</n-tag><n-button @click="load">刷新状态</n-button></n-space></n-card
  >
</template>
