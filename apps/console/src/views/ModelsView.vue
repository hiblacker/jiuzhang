...
const lastQuery = ref<Row | null>(null)
...
function route() {
  if (!project || !datasetId.value) {
    throw new Error('请先选择项目和数据集')
  }
...
async function load() {
  ...
  if (!datasets.value.some(d => d.id === datasetId.value)) {
...
function modelForm(version?: Row) {
  ...
  open(
    ...
    async (value) => {
      if (!/^[0-9a-f]{40}$/.test(String(value.gitRevision))) {
        throw new Error('Git 提交必须为 40 位十六进制值')
      }
      if (!/^[0-9a-f]{64}$/.test(String(value.bundleSha256))) {
        throw new Error('模型包摘要必须为 64 位十六进制值')
      }
  ...
  if (owner.value && row.state === 'READY') {
    items.push({
      ...
      run: () =>
        open(
          `发布候选 #${row.id}`,
          [{ key: 'reason', label: '发布理由', type: 'textarea', required: true }],
          {},
          v => call(`${path}/builds/${row.id}/publish`, v),
        ),
    })
  }
...
async function query(value: Row) {
  await run(async () => {
    ...
    if (value.releaseId) {
      body.releaseId = value.releaseId
    }
...
async function exportPage() {
  await run(async () => {
    if (!lastQuery.value || !result.value) {
      return
    }
...
<template>
  ...
  <template v-else>
    <div class="dataset-bar">
      ...
      </NTag><NTag v-if="selected?.active_release_id" type="success" :bordered="false">
    ...
    <section v-if="datasetId" class="section">
      <NTabs v-model:value="activeTab" type="line">
        ...
        >
          ...
          <div v-if="result" class="section-gap">
            <div class="toolbar">
              ...
              <NButton class="push" :disabled="loading" @click="exportPage">
                <template #icon>
                  <Download :size="16" />
                </template>导出当前页
              </NButton>
              ...
              >
                ...
                :disabled="
                  loading ||
                    ...
                    result.offset + result.limit > 100000
                "
                    ...
                    { label: '登记新版本', run: () => modelForm(row) },
                    {
                      ...
                      run: () =>
                        run(async () =>
                          inspect(
                            '变更影响',
                            await call(`${route()}/impact?version=${row.version}`),
                          ),
                        ),
                    },
                  ]
        ...
        >
          <div class="toolbar">
            <NButton type="primary" :disabled="loading || !versions.length" @click="buildForm">
              构建候选
            </NButton>
          </div>
          <DataGrid
        ...
        >
          <NButton :disabled="loading" @click="policyForm">
            配置数据授权
          </NButton>
        </NTabPane>
      </NTabs>
