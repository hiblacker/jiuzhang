...
async function load() {
  if (admin) {
    identities.value = await call<Row[]>('warehouse/identities')
  }
  if (project) {
    sources.value = await call<Row[]>(`warehouse/projects/${project}/sources`)
    if (owner.value) {
      members.value = await call<Row[]>(`warehouse/projects/${project}/members`)
    }
  }
}
...
<template>
  ...
  <section class="section">
    <NTabs v-model:value="activeTab" type="line">
      ...
      >
        <div class="toolbar">
          <NButton type="primary" :disabled="loading" @click="projectForm">
            <template #icon>
              <Plus :size="16" />
            </template>创建项目
          </NButton>
      ...
      >
        <div class="toolbar">
          <NButton type="primary" :disabled="loading" @click="identityForm">
            <template #icon>
              <Plus :size="16" />
            </template>创建身份
          </NButton>
        ...
        <DataGrid
          ...
          :actions="
            (row) => (row.enabled ? [{ label: '撤销', danger: true, run: () => revoke(row) }] : [])
          "
        />
      </NTabPane>
      <NTabPane
      ...
      >
        <NEmpty v-if="!project" description="暂无可访问的项目" /><template v-else>
          <div v-if="admin" class="toolbar">
            <NButton :disabled="loading" @click="bindSource">
              分配来源
            </NButton>
          </div>
          <DataGrid
      ...
      >
        <div class="toolbar">
          <NButton type="primary" :disabled="loading" @click="memberForm()">
            <template #icon>
              <Plus :size="16" />
            </template>添加成员
          </NButton>
