...
function open(title: string, items: Field[], value: Row, action: (v: Row) => Promise<unknown>) {
...
function projectForm() {
  open(
    ...
    {},
    (v) => call('warehouse/projects', v),
  );
}
function identityForm() {
  open('创建身份', [{ key: 'id', label: '身份编码', required: true }], {}, async (v) => {
    const r = await call<{ id: string; token: string }>('warehouse/identities', v);
    issuedIdentity.value = r.id;
    issuedToken.value = r.token;
    return r;
  });
}
function memberForm(row?: Row) {
  open(
    '保存成员角色',
    [
      { key: 'identity', label: '身份编码', required: true, disabled: !!row },
      {
        key: 'role',
        label: '角色',
        type: 'select',
    ...
    { identity: row?.identity_id ?? '', role: row?.role ?? 'VIEWER' },
    (v) => call(`warehouse/projects/${project}/members`, v),
  );
}
function bindSource() {
  open('分配来源', [{ key: 'sourceCode', label: '来源编码', required: true }], {}, (v) =>
    call(`warehouse/projects/${project}/sources`, v),
  );
}
function revoke(row: Row) {
  dialog.warning({
    ...
    onPositiveClick: () =>
      run(async () => {
        await call(`warehouse/identities/${encodeURIComponent(String(row.id))}/revoke`, {});
        await load();
      }),
  });
}
...
<template>
  ...
  <section class="section">
    <n-tabs v-model:value="activeTab" type="line">
      <n-tab-pane
        ...
        ><div class="toolbar">
          <n-button type="primary" :disabled="loading" @click="projectForm"
            ><template #icon><Plus :size="16" /></template>创建项目</n-button
          >
        </div>
        <DataGrid
          :rows="projects"
      ...
      <n-tab-pane
        ...
        ><div class="toolbar">
          <n-button type="primary" :disabled="loading" @click="identityForm"
            ><template #icon><Plus :size="16" /></template>创建身份</n-button
          >
        </div>
        <DataGrid
          :rows="identities"
          ...
          :actions="(row) => (row.enabled ? [{ label: '撤销', danger: true, run: () => revoke(row) }] : [])"
      /></n-tab-pane>
      <n-tab-pane name="sources" tab="来源归属" :tab-props="tabProps('sources')" v-bind="panelProps('sources')"
        ><n-empty v-if="!project" description="暂无可访问的项目" /><template v-else
          ><div v-if="admin" class="toolbar"><n-button :disabled="loading" @click="bindSource">分配来源</n-button></div>
          <DataGrid
            :rows="sources"
            :loading="loading"
            :columns="[
      ...
      <n-tab-pane
        ...
        ><div class="toolbar">
          <n-button type="primary" :disabled="loading" @click="memberForm()"
            ><template #icon><Plus :size="16" /></template>添加成员</n-button
          >
        </div>
        <DataGrid
          :rows="members"
          ...
          :actions="(row) => [{ label: '修改角色', run: () => memberForm(row) }]"
      /></n-tab-pane>
    </n-tabs>
  </section>
  <n-modal
  ...
  <n-modal
    ...
    />
    <div class="toolbar"><n-button type="primary" @click="issuedToken = ''">关闭</n-button></div></n-modal
  >
</template>
