...
test('built UI connects through its real same-origin proxy with memory-only credentials', async ({
  page,
}) => {
  const calls = []
  const errors = []
  ...
  const upstream = http.createServer((request, response) => {
    ...
    response.writeHead(request.url === '/api/v1/warehouse/projects' ? 200 : 404, {
  ...
  try {
    ...
    expect(calls).toEqual([
      {
        path: '/api/v1/warehouse/projects',
        token: 'Bearer synthetic-proxy-test-only',
        cookie: undefined,
      },
    ])
