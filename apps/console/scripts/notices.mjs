...
for (const [location, pkg] of Object.entries(lock.packages)) {
  ...
  const names = (await readdir(directory)).filter(name => /^(licen[sc]e|notice|copying)(\.|$)/i.test(name));
  ...
  if (!texts.length) {
    ...
    if (!supplement || pkg.license !== 'MIT') throw new Error(`MISSING_LICENSE_FILE: ${location}`);
    texts.push(await readFile(path.join(root, 'licenses', supplement), 'utf8'));
  }
  sections.push(`${location} @ ${pkg.version} (${pkg.license})\n\n${texts.join('\n\n')}`);
}
await writeFile(path.join(root, 'dist/third-party-notices.txt'), sections.join('\n\n' + '='.repeat(72) + '\n\n') + '\n');
