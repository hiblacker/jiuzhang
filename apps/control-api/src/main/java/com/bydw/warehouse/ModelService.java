...
public class ModelService {
  ...
  public ModelService(JdbcTemplate jdbc, ObjectMapper json, ProductAccessService access) { this.jdbc = jdbc; this.json = json; this.access = access; }
  ...
  public Map<String, Object> save(long project, JsonNode r, String actor) {
    ...
    jdbc.update("INSERT INTO warehouse.model_version(dataset_id, version, runtime_ref, git_revision, bundle_sha256, contract, created_by) VALUES (?, ?, ?, ?, ?, ?::jsonb, ?)", id, current + 1, ref, r.path("gitRevision").asText(), r.path("bundleSha256").asText(), r.path("contract").toString(), actor);
  ...
  public List<Map<String, Object>> versions(long project, long id, String actor) {
    ...
    var rows = jdbc.queryForList("SELECT version, runtime_ref, git_revision, bundle_sha256, contract, created_by, created_at FROM warehouse.model_version WHERE dataset_id = ? ORDER BY version DESC LIMIT 200", id);
  ...
  public Map<String, Object> build(long project, long dataset, JsonNode request, String actor) {
    ...
    for (var definition : contract.path("inputs")) {
      ...
      if (assetId.startsWith("mysql:")) rows = jdbc.queryForList("""
          SELECT o.raw_path AS path, o.raw_sha256 AS sha256, o.byte_count AS bytes, o.row_count AS rows,
            ...
            r.manifest_json AS manifest FROM lake.object_run o JOIN lake.system_run r ON r.id = o.system_run_id
      ...
      inputs.add(input); watermark.addObject().put("alias", alias).put("at", ((java.sql.Timestamp) row.get("watermark")).toInstant().toString()).put("assetId", assetId).put("assetVersion", id);
    }
    ...
    long id = jdbc.queryForObject("INSERT INTO warehouse.model_build(dataset_id, model_version, expected_release_id, inputs, watermark, request_key, state) VALUES (?, ?, ?, ?::jsonb, ?::jsonb, ?, 'QUEUED') RETURNING id", Long.class, dataset, version, d.get("active_release_id"), inputs.toString(), watermark.toString(), request.path("requestKey").asText());
