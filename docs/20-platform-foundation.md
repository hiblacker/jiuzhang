...
尚未实现：多用户/SSO、来源连接测试、DevOps 真实抽取、ODS、质量门禁、数据集发布服务、Naive UI、NAS 部署、备份恢复和容量测试。独立 Worker 进程已能用受限库账号写入合成 RAW，但不能连接业务 MySQL。后续实施顺序、出口证据和阻断项见[后续规划与里程碑](21-next-milestones.md)。
