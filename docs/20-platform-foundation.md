...
| 控制 API 镜像 | `bydw/control-api:0.1.0-dev.1` | 当前开发构建版本；每次代码变化递增末尾序号，正式发布时改为对应 SemVer |
...
直接版本已固定；Spring Boot BOM 管理的传递版本仍需在交付前生成完整 SBOM、许可证和漏洞报告。Docker 使用唯一版本标签而非 digest；已被容器使用的开发标签不得由后续构建覆盖。Maven 构建使用不进入运行镜像的 BuildKit 缓存，以便网络中断后继续下载。发布时另行记录构建结果和扫描结果。当前锁定不等于外部私有化交付许可已经批准。
...
首版迁移包含来源、接入任务/批次、数据集版本/发布/活动指针和审计日志的最小表。后续 V002–V006 已增加任务契约、批次检查点、RAW 完成证据和重试/取消状态；详见[来源登记 API](22-source-registry.md)、[批次状态机](24-ingestion-batch-checkpoint.md)、[RAW 批次门禁](25-raw-batch-evidence.md)和[批次重试取消](26-batch-retry-cancel.md)。长时间接入和模型执行不会放进 HTTP 请求处理器。
