...
首版迁移包含来源、接入任务/批次、数据集版本/发布/活动指针和审计日志的最小表。后续 V002–V005 已增加任务契约、批次检查点和 RAW 完成证据；详见[来源登记 API](22-source-registry.md)、[批次状态机](24-ingestion-batch-checkpoint.md)和[RAW 批次门禁](25-raw-batch-evidence.md)。长时间接入和模型执行不会放进 HTTP 请求处理器。
