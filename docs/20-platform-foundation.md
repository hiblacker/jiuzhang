...
直接版本已固定；Spring Boot BOM 管理的传递版本仍需在交付前生成完整 SBOM、许可证和漏洞报告。Docker 使用版本标签而非 digest；发布时另行记录构建摘要和扫描结果。当前锁定不等于外部私有化交付许可已经批准。
...
首版迁移包含来源、接入任务/批次、数据集版本/发布/活动指针和审计日志的最小表。API 当前只提供服务状态接口，长时间接入和模型执行不会放进 HTTP 请求处理器。
...
尚未实现：身份与授权、来源登记 API、运行控制、真实 DevOps 抽取、RAW/ODS、质量门禁、数据集发布服务、Naive UI、NAS 部署、备份恢复和容量测试。后续实施顺序、出口证据和阻断项见[后续规划与里程碑](21-next-milestones.md)。
