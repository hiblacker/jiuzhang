...
打开 `http://127.0.0.1:60284`，选择“使用邀请码开户 / 重置密码”，从生成的私有邀请文件取值并设置密码，再用账号登录。邀请码固定 43 位，只复制 `invitation` 的值：带引号、花括号、换行或空白都不是有效邀请码；页面先修剪首尾空白再本地校验位数，不合法时直接提示且不发出请求。密码至少 12 个字符且不超过 72 字节（一个汉字按 3 字节计）。邀请 24 小时有效、仅使用一次。页面不要求填管理令牌。60283 与 60284 都映射同一 Java 服务；选定一个地址持续使用，Cookie 按同源会话工作。新环境尚无业务来源和模型任务。

开户/重置的失败原因分开返回，便于区分用户输入和服务状态：`INVALID_INVITATION_FORMAT`（邀请码位数或字符不合法）、`INVALID_PASSWORD_FORMAT`（密码字符数不足 12 或超过 72 字节）、`INVITATION_EXPIRED`（邀请码已使用、已过期或不存在）。三者都不回显提交的邀请码或密码。
...
2. **登记数据源并建立采集数据集。** 一个系统实例可同时有 MySQL、目录和 REST 数据源；同一数据源可建立多个独立采集数据集。页面不接收宿主绝对路径、任意 URL 或密码。
...
负责人在“项目与设置”邀请账号或创建独立服务身份，在“服务数据集”配置允许字段和多个同时满足的行条件。查看者选择固定 release、字段、类型化筛选/排序查询，CSV 导出当前有界页；分页始终携带同一 release。查询审计保存身份、发布/策略版本、条件摘要、行数、耗时与请求 ID，不保存筛选原值和查询数据。
