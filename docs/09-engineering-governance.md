...
每次发布一个清单：Git SHA、应用版本、各镜像版本标签、依赖锁文件摘要、配置版本、数据库迁移目标、工作流/模型/指标版本、SBOM和测试结果。开发→测试→生产晋级同一镜像版本，不在每个环境重新构建不同镜像。

软件制品发布清单与数据release_id分开：同一软件版本可生成多次业务数据发布。数据库迁移单独有V001/V002等序号、校验和及执行状态，已经执行的迁移不修改原文件。

依赖清单至少登记组件/模块、用途、上游地址、精确版本、非镜像制品校验值、直接/传递依赖、许可证、NOTICE、漏洞处置、维护者、批准状态。镜像按版本标签登记和运行，不以镜像 ID 或 digest 代替版本。没有明确许可证不进入批准状态；运行时/JDBC驱动/连接器/浏览器和测试工具也在范围内。
