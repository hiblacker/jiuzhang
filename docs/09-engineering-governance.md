...
使用锁定的 Release/tag 或经审核的 commit，不使用 `latest`；Docker Compose/Dockerfile 使用明确版本标签，不使用 `@sha256` 镜像引用；同时保存可选的镜像摘要、SBOM、许可证/NOTICE和漏洞扫描结果。升级先在独立分支做兼容性、数据回放、性能和恢复测试，再生成变更记录。
...
Docker是打包和交付格式，不是高可用、权限、数据持久化或发布一致性的替代品。镜像使用明确版本标签并在发布清单中记录构建时间、Git SHA和可选摘要；配置与镜像分离；数据库迁移遵循向前兼容，回滚优先回滚应用或数据集发布指针，避免破坏性“降级数据库”。
...
- 回滚：应用镜像按已发布版本标签回滚，并核对发布清单；数据发布切回旧release；任务重算使用明确版本，不盲目回滚已写入的事实数据。
...
每次发布一个清单：Git SHA、应用版本、各镜像版本标签、可选镜像摘要、依赖锁文件摘要、配置版本、数据库迁移目标、工作流/模型/指标版本、SBOM和测试结果。开发→测试→生产晋级同一镜像版本，不在每个环境重新构建不同镜像。
