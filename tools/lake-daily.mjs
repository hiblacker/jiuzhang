...
import { atomicJson, currentDay, validateDay, withLock } from './lake-runtime.mjs';
...
async function runChild(script, args) {
  ...
  child.stdout.on('data', (chunk) => { stdout = (stdout + chunk.toString('utf8')).slice(0, 20000); });
  ...
  if (code !== 0) {
    let detail; try { detail = JSON.parse(stderr.trim()); } catch {}
    const error = new Error(detail?.errorCode ?? parsed?.errorCode ?? 'DAILY_CHILD_FAILED');
    error.stdout = stdout; error.stderr = stderr;
    throw error;
  }
  if (!parsed || !['COMPLETE', 'INCOMPLETE', 'DRY_RUN', 'NOT_OBSERVED'].includes(parsed.state)) fail('DAILY_CHILD_PROTOCOL_FAILED');
  return parsed;
}
...
export async function run(options, execute = runChild) {
  ...
  try {
    ...
    try { runManifest.tasks.mysql = await execute(path.join(ROOT, 'tools/lake-ingest.mjs'), mysqlArgs); }
    catch (error) { runManifest.tasks.mysql = { state: 'FAILED', errorCode: error.message }; }

    if (options.register) {
      if (options.dryRun) runManifest.tasks.register = { state: 'DRY_RUN' };
      else if (runManifest.tasks.mysql.batchId) {
        ...
        try { runManifest.tasks.register = await execute(path.join(ROOT, 'tools/lake-register.mjs'), registerArgs); }
    ...
    if (options.inbox) {
      ...
      try { runManifest.tasks.files = await execute(path.join(ROOT, 'tools/file-ingest.mjs'), fileArgs); }
    ...
    if (options.apiConfig) {
      ...
      try { runManifest.tasks.api = await execute(path.join(ROOT, 'tools/rest-ingest.mjs'), apiArgs); }
    ...
    return runManifest;
  } finally { /* per-source tools own their immutable evidence and locks */ }
  };
  return options.dryRun ? executeTasks() : withLock(path.join(options.lakeRoot, 'daily-run.lock'), executeTasks);
}
