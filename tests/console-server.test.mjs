...
test('console serves built assets, enforces proxy scope and retains auth only upstream', async () => {
  ...
  const upstream = http.createServer(async (request, response) => {
    ...
    calls.push({ url: request.url, token: request.headers.authorization, cookie: request.headers.cookie, body });
  ...
  try {
    ...
    const query = await fetch(base + '/api/v1/warehouse/projects?scope=1', { method: 'POST', headers: { Authorization: 'Bearer synthetic-only', Cookie: 'do-not-forward=1' }, body: '{"test":true}' });
    assert.equal(query.status, 200);
    assert.deepEqual(calls, [{ url: '/api/v1/warehouse/projects?scope=1', token: 'Bearer synthetic-only', cookie: undefined, body: '{"test":true}' }]);
