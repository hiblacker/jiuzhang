...
test('manifest registration maps committed files to relative immutable objects', async () => {
  ...
  try {
    ...
    const manifest = { batchId: 'batch-1', sourceCode: 'mysql-test-source', mode: 'FULL', state: 'COMPLETE', consistency: 'ONE_REPEATABLE_READ_TRANSACTION', startedAt: '2026-09-15T00:00:00Z', finishedAt: '2026-09-15T00:01:00Z', tables: [{ table: 'orders', state: 'RAW_COMMITTED', rowCount: 1, bytes: 5, sha256: 'b'.repeat(64) }] };
