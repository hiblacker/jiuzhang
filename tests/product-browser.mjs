...
try{
  ...
  await menu('数据服务');await button('订单明细').click();await button('配置行列授权').click();await select('项目查看者 / 服务身份',id+'_viewer');await select('允许查询字段','order_id');await page.keyboard.press('Escape');await select('允许查询字段','amount');await page.keyboard.press('Escape');await button('增加行条件').click();await modal().locator('.n-base-selection').last().click();await page.locator('.n-base-select-option:visible').filter({hasText:'team'}).click();await modal().getByPlaceholder('字段等于此值').fill('east');await button('增加行条件').click();await modal().locator('.n-base-selection').last().click();await page.locator('.n-base-select-option:visible').filter({hasText:'order_id'}).click();await modal().getByPlaceholder('字段等于此值').nth(1).fill('001');await button('保存授权新版本').click();
  await button('配置行列授权').click();await select('项目查看者 / 服务身份',id+'_viewer');assert.deepEqual((await modal().getByPlaceholder('字段等于此值').evaluateAll(inputs=>inputs.map(input=>input.value))).sort(),['001','east']);await button('保存授权新版本').click();
