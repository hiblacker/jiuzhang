...
test('daily orchestrator requires an explicit valid window when supplied', () => {
  ...
  assert.throws(() => parseArgs(['--mysql-source-code', '../outside']), /INVALID_SOURCE_CODE/);
});

test('a configured inbox without observed delivery cannot report the whole day complete', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'lake-missing-delivery-'));
  try {
    await assert.rejects(run(parseArgs(['--lake-root', root, '--inbox', '/configured/inbox']),
      async script => ({ state: script.endsWith('file-ingest.mjs') ? 'NOT_OBSERVED' : 'COMPLETE' })), /DAILY_RUN_INCOMPLETE/);
